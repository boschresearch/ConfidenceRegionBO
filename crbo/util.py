...
def theta_to_mlp_model(theta, model):
    """From the vectorized weights fill last linear layer of the model's policy.
    
    :param model: (BaseRLModel) Model to be updated.
    :param theta: (np.ndarray) Vector containing the policy parameters.

    :return: (BaseRLModel) Model with updated policy.    
    """
    if isinstance(model.env.action_space, gym.spaces.Box):
        action_dim = model.env.action_space.shape[0]
    elif isinstance(model.env.action_space, gym.spaces.Discrete):
        action_dim = model.env.action_space.n
    else:
        raise NotImplementedError()

    theta = theta.squeeze()
    param_dict = model.get_parameters()
    param_dict["model/pi/w:0"] = theta[:-action_dim].reshape((-1, action_dim))
    param_dict["model/pi/b:0"] = theta[-action_dim:]
